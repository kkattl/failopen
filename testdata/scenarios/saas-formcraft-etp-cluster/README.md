...
**Mutation of [`saas-formcraft`](../saas-formcraft/) — exactly one change:** the gateway NodePort Service uses `externalTrafficPolicy: Cluster` (the Kubernetes default) instead of `Local`: every node forwards, and kube-proxy masquerades the source.
